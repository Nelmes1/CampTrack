...
from logistics_coordinator_features import dashboard, plot_food_stock, plot_camper_distribution, plot_leaders_per_camp, plot_engagement_scores
...
def assigning_camp_to_supervise(leader_username):
    while True:
        camps = read_from_file()
        if camps == []:
...
def run(leader_username):
    while True:
        ...
        if choice == 1:
            assigning_camp_to_supervise(leader_username)
